<template>
  <div class="mt-2 sm:w-11/12 sm:m-auto sm:mt-2 md:w-full md:px-5">
    <div class="flex justify-between">
      <h3 class="heading">Completed Manga</h3>
    </div>

    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
      <div v-for="manga in completedMangas" :key="manga.mal_id">
...
export default {
  ...
  mounted() {
    this.fetchCompletedMangas();
  },
  methods: {
    async fetchCompletedMangas() {
      try {
        this.isLoading = true;
        const response = await fetch(`/api/manga/completed?page=${this.page}`);
        const data = await response.json();

        this.completedMangas.push(...data.data);
